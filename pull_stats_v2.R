pull_stats <- function(file, away = c(TRUE, FALSE), row_names = c(TRUE, FALSE), skip, nrows){
  ...
  require(data.table)
    ...
    links <- read.table(file = file, sep = "", skip = skip, nrows = nrows)
  ...
  # Bug Fix, Specifying Var Name as NA then feeding TEAM/OPPONENT names.
  
  game_diffs$Opponent <- NA
